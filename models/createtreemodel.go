...
func CreateComponentTreeJson() string {
	...
	for _, org := range orgs {
		...
		childorgs := make([]string, 0)
		for _, org21 := range orgs2 {
			if org21.Parentid == org.Componentname {
				childorgs = append(childorgs, org21.Componentname)
				//处理过的移除，减少下次循环反而更慢，估计和slice收缩有关
				//orgs2 = Remove2(orgs2, idx, idx)
			}
		...
		fmt.Println(childorgs)
		if childorgs != nil && len(childorgs) > 0 {
			node := Node{Key: org.Componentname, Parentid: org.Parentid, Title: org.Title, Checked: "false", Childrens: childorgs}
			m[org.Componentname] = node
		} else {
			node := Node{Key: org.Componentname, Parentid: org.Parentid, Title: org.Title, Checked: "false"}
			m[org.Componentname] = node
		}
