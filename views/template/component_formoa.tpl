...
export class ${firstuppercomponentname$}Component implements OnInit {
  ...
  dosubmit(event) {
    ...
    this.es.save${componentname$}(this.formdata.Opinion, this.ls.userid, this.modualid, this.currentfiid, this.currenttiid, event, this.formdata).subscribe(data => {
      console.log(data);
    ...
    for (const colname of this.formcolnames) {
      if (colname.Controltype == 'upload') {
        ...
        if (colname.fileList.length < colname.limitfileqty) {
          ...
          this.message.info('permitted max file quantity is :' + colname.limitfileqty);
          break;
        }
      }
    }
