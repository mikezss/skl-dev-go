...
//获得数据条数
func Get${componentname$}count() (page PAGE, err error) {
	o := orm.NewOrm()
	sql := "SELECT count(1) as total  from skl_${componentname$}_tb a" 		
	err = o.Raw(ConvertSQL(sql, Getdbtype())).QueryRow(&page)
	return page, err
}
//获得分页数据
func Get${componentname$}bypageindex(l PAGE) (admins []${uppercomponentname$}, err error) {
	dbtype := Getdbtype()
	admins = make([]${uppercomponentname$}, 0)
	o := orm.NewOrm()
